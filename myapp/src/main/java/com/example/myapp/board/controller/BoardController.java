...
public class BoardController {
	...
	public String getBoardDetails(@PathVariable int boardId, @PathVariable int page, Model model) {
		...
		if (fileName != null) {
			...
			String fileType = fileName
					.substring(fileLength-4, fileLength)
					.toUpperCase();
	...
	public String writeArticle(
		Board board, BindingResult results, RedirectAttributes redirectAttrs
	) {
		try {
	...
	public String deleteArticle(
			Board board, HttpSession session, RedirectAttributes model
	) {
		try {
			String dbpw = boardService.getPassword(board.getBoardId());
			System.out.println("dbpw 비밀번호 조회 " + dbpw);
			System.out.println("board 전송: " + board.toString());
			if (dbpw.equals(board.getPassword())) {
				...
				return "redirect:/board/cat/" + board.getBoardId() + "/" + page;
			} else {
